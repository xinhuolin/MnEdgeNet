...
class Model(torch.nn.Module):
    def __init__(self, lrate, loss_fn, in_size, out_size):
        ...
        self.conv1 = torch.nn.Conv1d(1, 64, 13)
        self.pool1 = torch.nn.MaxPool1d(2, 2)

        self.conv2 = torch.nn.Conv1d(64, 64, 13)
        self.pool2 = torch.nn.MaxPool1d(2, 2)

        self.conv3 = torch.nn.Conv1d(64, 128, 7)
        ...
        self.normalize = torch.nn.LayerNorm(3)

    def forward(self, x):
        x = torch.reshape(x, (-1, 1, 388))
...
def fit(train_data, test_set, epoch, batch_size=32):
    ...
    numpy.savetxt('EELSProject/mean3.csv', mean.numpy(), delimiter=',')
    numpy.savetxt('EELSProject/std3.csv', std.numpy(), delimiter=',')
    ...
    standardize = standardize.cuda()
    torch.save(model.state_dict(), 'EELSProject/EELS4.pt')
    model.eval()
    index = 0
