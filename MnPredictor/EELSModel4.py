...
def fit(train_data, test_set, epoch, batch_size=32):
    ...
    model = Model(0.00008, torch.nn.MSELoss(), 388, 4).cuda()
    losses = []
    yhat = []
    index = 0
    ...
    for i in range(epoch):
        ...
        train_label = train_data[:, 0:4]
        train_set = train_data[:, 4:]
        train_set = torch.tensor(train_set, dtype=torch.float32)
        ...
        while 1:
            ...
            index += batch_size
    torch.cuda.empty_cache()
    mean = test_set.mean(dim=1, keepdim=True)
    std = test_set.std(dim=1, keepdim=True)
    standardize = (test_set - mean) / std
    standardize = standardize.cuda()
    torch.save(model.state_dict(), 'EELSProject/EELS8.pt')
    model.eval()
    index = 0
